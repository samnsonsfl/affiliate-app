...
const defaultApps = [
  { id: "store",  name: "Store",          desc: "Gear we actually use.",           href: "#" , badge: "Starter" },
  { id: "travel", name: "Travel Planner", desc: "Trips, tips, and tools.",         href: "#" , badge: "Starter" },
  { id: "work",   name: "Work Hub",       desc: "Docs & tasks in one place.",      href: "#" , badge: "Starter" },
  { id: "life",   name: "Life Admin",     desc: "Bills, reminders, and more.",     href: "#" , badge: "Starter" },
];
...
function ErrorNote({children}){
  return (
    <div className="au-card" style={{borderColor: "rgba(248,113,113,.5)", background:"rgba(248,113,113,.12)", padding:12, marginBottom:8}}>
...
function App(){
  ...
  // Boot session with sliding window
  useEffect(()=>{
    const s = loadSession();
      ...
      <div className="au-grid" style={{maxWidth:520, margin:"0 auto"}}>
        <div className="au-card">
          ...
          <div className="au-card-content">
            ...
            <form onSubmit={handleLogin} className="au-grid">
              <div>
                <label className="au-note">Email</label>
                <input className="au-input" type="email" placeholder="you@example.com"
              ...
              <div>
                ...
                <input className="au-input" type="password" placeholder="••••••••"
              ...
              <div className="au-row-between">
                <label className="au-row" style={{fontSize:14}}>
                  <input type="checkbox" checked={loginForm.stay} onChange={(e)=>setLoginForm({...loginForm, stay:e.target.checked})} />
                  <span>Stay signed in for 30 days</span>
                </label>
                <button type="button" className="au-btn au-btn-secondary" onClick={()=>setRoute("signup")}>Create account</button>
              </div>
    ...
    <Shell route={route} onLogout={handleLogout}>
      <div className="au-grid" style={{maxWidth:720, margin:"0 auto"}}>
        <div className="au-card">
          ...
          <div className="au-card-content">
            {err && <ErrorNote>{err}</ErrorNote>}
            <form onSubmit={handleSignup} className="au-grid au-grid-2">
              <div className="au-row" style="flex-direction:column; align-items:stretch; grid-column:1/-1;">
                <label className="au-note">Full name</label>
                <input className="au-input" placeholder="Jane Doe"
              ...
              <div>
                <label className="au-note">Password</label>
                <input className="au-input" type="password" placeholder="Min 8 characters"
              ...
              <div>
                ...
                <input className="au-input" type="password" placeholder="Repeat password"
                  value={signupForm.confirm} onChange={(e)=>setSignupForm({...signupForm, confirm:e.target.value})} />
              </div>
              <div className="au-row" style="grid-column:1/-1;">
                <label className="au-row" style={{fontSize:14}}>
                  <input type="checkbox" checked={signupForm.agree} onChange={(e)=>setSignupForm({...signupForm, agree:e.target.checked})} />
                  <span>I agree to the <a href="#" onclick="event.preventDefault()">Terms & Conditions</a>.</span>
                </label>
              </div>
              <div className="au-row" style="grid-column:1/-1;">
                <label className="au-row" style={{fontSize:14}}>
                  <input type="checkbox" checked={signupForm.optIn} onChange={(e)=>setSignupForm({...signupForm, optIn:e.target.checked})} />
                  <span>Send me helpful updates and the occasional ✨ good spam ✨</span>
                </label>
              </div>
              <div className="au-row" style="grid-column:1/-1; gap:12px;">
                <button type="submit" className="au-btn au-btn-primary">Create account</button>
                <button type="button" className="au-btn au-btn-secondary" onClick={()=>setRoute("login")}>I already have an account</button>
              </div>
            </form>
          </div>
          <div className="au-card-footer">
  ...
  const Dashboard = () => {
    ...
    // 👉 If you want a tile that opens your existing affiliate index
    //     change href below to "./affiliate.html" or keep "#"
    return (
      <Shell route={route} onLogout={handleLogout}>
            ...
            {apps.map(app => (
              <div key={app.id} className="au-card">
                ...
                <div className="au-card-footer au-row" style={{gap:12}}>
                  <a className="au-btn au-btn-primary" href={app.href}>Open</a>
  ...
  if (route === "loading") return <div className="au-container" style={{display:"grid", placeItems:"center", minHeight:"40vh"}}>Loading…</div>;
